...
const s = {
  ...
  btnG: { background: "rgba(255,255,255,0.08)", border: "1px solid rgba(255,255,255,0.15)", color: "#FFFFFF", padding: "9px 18px", borderRadius: 8, cursor: "pointer", fontFamily: "inherit", fontWeight: 700, fontSize: 14 },
    ...
    <div style={{ minHeight: "100vh", background: "#0A0F1E", display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "'Cairo',sans-serif", direction: "rtl" }}>
      <div style={{ background: "#0F172A", border: "1px solid #1E3A5F", borderRadius: 16, padding: 32, width: 340 }}>
        <div style={{ textAlign: "center", marginBottom: 24 }}>
          ...
          <div style={{ fontSize: 12, color: "#FFFFFF" }}>سجل دخول للمتابعة</div>
        </div>
  ...
  return (
    <div style={s.wrap}>
      ...
      <div style={s.header} className="no-print">
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          ...
          <div>
            ...
            <div style={{ fontSize: 11, color: "#FFFFFF" }}>أهلاً {currentUser.name} {isAdmin ? "👑" : ""}</div>
          </div>
        </div>
          ...
          <div style={{ position: "relative" }}>
            ...
            {showNotifications && (
              <div style={{ position: "absolute", top: 44, left: 0, background: "#111827", border: "1px solid #1E3A5F", borderRadius: 10, width: 280, maxHeight: 300, overflowY: "auto", zIndex: 300, padding: 8 }}>
                {notifications.length === 0 && <div style={{ color: "#FFFFFF", fontSize: 12, textAlign: "center", padding: 12 }}>لا توجد إشعارات</div>}
                {notifications.slice(0, 10).map(n => (
                  <div key={n.id} onClick={() => markNotifRead(n.id)} style={{ padding: "8px 10px", borderRadius: 7, marginBottom: 4, background: n.read ? "transparent" : "#1E3A5F22", cursor: "pointer", borderRight: n.read ? "none" : "3px solid #2563EB" }}>
                    <div style={{ fontSize: 12, color: n.read ? "#FFFFFF99" : "#FFFFFF" }}>{n.message}</div>
                    <div style={{ fontSize: 10, color: "#FFFFFF" }}>{n.time?.toDate ? n.time.toDate().toLocaleString("ar-EG") : ""}</div>
                  </div>
                ))}
              </div>
            )}
      ...
      <div style={{ padding: "12px 20px 0", display: "flex", gap: 6, flexWrap: "wrap" }} className="no-print">
        {TABS.map(t => (
          <button key={t.key} onClick={() => setTab(t.key)} style={{
            ...
            color: "#FFFFFF", cursor: "pointer", fontFamily: "inherit", fontWeight: 600, fontSize: 12
          }}>{t.label}</button>
        ))}
      </div>
      ...
      <div style={{ padding: "16px 20px" }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(120px,1fr))", gap: 12, marginBottom: 20 }}>
              {[
                ...
                <div key={i} style={{ background: "#0F172A", border: "1px solid #1E3A5F", borderRadius: 10, padding: 14, textAlign: "center" }}>
                  ...
                  <div style={{ fontSize: 11, color: "#FFFFFF", marginTop: 4 }}>{st.label}</div>
                </div>
              ))}
            </div>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(220px,1fr))", gap: 16 }}>
              <div style={s.card}>
                <div style={{ fontSize: 13, fontWeight: 700, color: "#FFFFFF", marginBottom: 14 }}>توزيع المهام</div>
                {STATUSES.map(st => {
              ...
              <div style={s.card}>
                <div style={{ fontSize: 13, fontWeight: 700, color: "#FFFFFF", marginBottom: 14 }}>إنجاز الموظفين</div>
                {employees.map(e => {
                  ...
                  return (
                    <div key={e.id} style={{ marginBottom: 12 }}>
                      <div style={{ display: "flex", justifyContent: "space-between", fontSize: 12, marginBottom: 4 }}>
                        <span style={{ color: "#FFFFFF" }}>{e.name}</span>
              ...
              <div style={s.card}>
                <div style={{ fontSize: 13, fontWeight: 700, color: "#FFFFFF", marginBottom: 14 }}>⚠️ تنبيهات المواعيد</div>
                {alerts.length === 0 && <div style={{ color: "#FFFFFF", fontSize: 12, textAlign: "center", padding: "20px 0" }}>لا توجد تنبيهات</div>}
                {alerts.map(t => {
                  ...
                  return (
                    <div key={t.id} style={{ background: "#1A0F0F", border: "1px solid #3B1F1F", borderRadius: 8, padding: 10, marginBottom: 8 }}>
                      ...
                      <div style={{ display: "flex", gap: 8 }}>
                        ...
                        <span style={{ fontSize: 11, color: "#FFFFFF" }}>📅 {t.due}</span>
                      </div>
                    </div>
                  );
                })}
              ...
              {STATUSES.map(status => (
                <div key={status} style={{ ...s.card, minWidth: 240, flex: "0 0 240px" }}>
                  ...
                  <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                        ...
                        <div key={task.id} className="task-card" style={{ background: "#111827", border: `1px solid ${urg ? urg.color + "55" : "#1E293B"}`, borderRadius: 10, padding: 12, cursor: "pointer" }} onClick={() => setSelectedTask(task)}>
                          ...
                          {emp && (
                            <div style={{ display: "flex", alignItems: "center", gap: 5, marginBottom: 4 }}>
                              ...
                              <span style={{ fontSize: 10, color: "#FFFFFF" }}>{emp.name}</span>
                              {client && <span style={{ fontSize: 10, color: "#FFFFFF" }}>• {client.name}</span>}
                            </div>
                          )}
                          {task.tags?.length > 0 && (
                          ...
                          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginTop: 4 }}>
                            <div style={{ display: "flex", gap: 6 }}>
                              {task.due && <span style={{ fontSize: 9, color: urg ? urg.color : "#FFFFFF" }}>📅 {task.due}</span>}
                              {taskComments.length > 0 && <span style={{ fontSize: 9, color: "#FFFFFF" }}>💬 {taskComments.length}</span>}
                            </div>
                          </div>
                          {canEdit && (
                    ...
                    {byStatus(status).length === 0 && <div style={{ color: "#FFFFFF", fontSize: 11, textAlign: "center", padding: "12px 0" }}>لا توجد مهام</div>}
                  </div>
                </div>
              ))}
            </div>
                ...
                <div key={emp.id} style={{ ...s.card, border: `1px solid ${emp.color}33` }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 14 }}>
                    ...
                    <div style={{ flex: 1 }}>
                      ...
                      <div style={{ fontSize: 11, color: "#FFFFFF" }}>@{emp.username}</div>
                    </div>
                    {isAdmin && (
                  ...
                  <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 6 }}>
                    <span style={{ fontSize: 11, color: "#FFFFFF" }}>الإنجاز</span>
                  ...
                  <div key={client.id} style={s.card}>
                    <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 12 }}>
                      ...
                      <div style={{ flex: 1 }}>
                        ...
                        <div style={{ fontSize: 11, color: "#FFFFFF" }}>{client.phone}</div>
                      </div>
                      <div style={{ display: "flex", gap: 5 }}>
        ...
        {tab === "payments" && (
          <div>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16, flexWrap: "wrap", gap: 8 }}>
              <div style={{ fontSize: 14, fontWeight: 700, color: "#FFFFFF" }}>💰 كشف المدفوعات</div>
              <div style={{ display: "flex", gap: 8 }}>
            ...
            <div style={s.card}>
                ...
                <thead>
                  <tr style={{ borderBottom: "1px solid #1E293B" }}>
                    {["العميل", "المبلغ", "النوع", "الحالة", "التاريخ", "بواسطة", ""].map((h, i) => (
                      <th key={i} style={{ padding: "8px 10px", color: "#FFFFFF", fontWeight: 600, textAlign: "right" }}>{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {payments.sort((a, b) => (b.date || "") > (a.date || "") ? 1 : -1).map(p => {
                    ...
                    return (
                      <tr key={p.id} style={{ borderBottom: "1px solid #0F172A" }}>
                        <td style={{ padding: "8px 10px", color: "#FFFFFF" }}>{client?.name || "-"}</td>
                        <td style={{ padding: "8px 10px", color: "#34D399", fontWeight: 700 }}>{parseFloat(p.amount).toLocaleString()} ج</td>
                        <td style={{ padding: "8px 10px", color: "#FFFFFF" }}>{p.type}</td>
                        ...
                        <td style={{ padding: "8px 10px", color: "#FFFFFF" }}>{p.date}</td>
                        <td style={{ padding: "8px 10px", color: "#FFFFFF" }}>{p.createdBy}</td>
                        <td style={{ padding: "8px 10px" }}>
              ...
              {payments.length === 0 && <div style={{ color: "#FFFFFF", textAlign: "center", padding: 20, fontSize: 13 }}>لا توجد مدفوعات بعد</div>}
            </div>
          </div>
        )}
        ...
        {tab === "activity" && (
          <div style={s.card}>
            <div style={{ fontSize: 14, fontWeight: 700, color: "#FFFFFF", marginBottom: 16 }}>📜 سجل النشاط</div>
            {activityLog.length === 0 && <div style={{ color: "#FFFFFF", textAlign: "center", padding: 20 }}>لا يوجد نشاط بعد</div>}
            {activityLog.map(log => (
              <div key={log.id} style={{ display: "flex", gap: 12, padding: "10px 0", borderBottom: "1px solid #1E293B" }}>
                ...
                <div>
                  <div style={{ fontSize: 13, color: "#FFFFFF" }}>
                    ...
                    {log.details && <span style={{ color: "#FFFFFF" }}> "{log.details}"</span>}
                  </div>
                  <div style={{ fontSize: 11, color: "#FFFFFF", marginTop: 2 }}>{log.time?.toDate ? log.time.toDate().toLocaleString("ar-EG") : "الآن"}</div>
                </div>
              </div>
            ))}
          </div>
        ...
        {tab === "calendar" && (
          <div style={s.card}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(7,1fr)", gap: 4, marginBottom: 8 }}>
              {DAYS_AR.map(d => <div key={d} style={{ textAlign: "center", fontSize: 11, color: "#FFFFFF", padding: "4px 0" }}>{d}</div>)}
            </div>
              ...
              {days.map((day, i) => {
                ...
                return (
                  <div key={i} style={{ background: isToday ? "#1E3A5F" : "#111827", border: `1px solid ${isToday ? "#60A5FA" : "#1E293B"}`, borderRadius: 8, padding: "6px 4px", minHeight: 60 }}>
                    <div style={{ fontSize: 12, color: isToday ? "#60A5FA" : "#FFFFFF", fontWeight: isToday ? 700 : 400, marginBottom: 4, textAlign: "center" }}>{day}</div>
                    ...
                    {dayTasks.length > 2 && <div style={{ fontSize: 9, color: "#FFFFFF" }}>+{dayTasks.length - 2}</div>}
                  </div>
                );
              })}
            </div>
        ...
        {tab === "reports" && (
          <div>
            <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 16, flexWrap: "wrap", gap: 8 }}>
              <div style={{ fontSize: 14, fontWeight: 700, color: "#FFFFFF" }}>📈 التقارير</div>
              <div style={{ display: "flex", gap: 8 }}>
                <button style={s.btnG} onClick={exportCSV}>📥 Excel</button>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(220px,1fr))", gap: 16 }}>
              <div style={s.card}>
                <div style={{ fontSize: 13, fontWeight: 700, color: "#FFFFFF", marginBottom: 14 }}>أداء الموظفين</div>
                {employees.map(e => {
                  const total = tasks.filter(t => t.empId === e.id).length;
                  const done = tasks.filter(t => t.empId === e.id && t.status === "تم الإنجاز").length;
                  ...
                  return (
                    <div key={e.id} style={{ marginBottom: 12, padding: 10, background: "#111827", borderRadius: 8 }}>
                      <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 6 }}>
                        <span style={{ color: "#FFFFFF", fontWeight: 600 }}>{e.name}</span>
              ...
              <div style={s.card}>
                <div style={{ fontSize: 13, fontWeight: 700, color: "#FFFFFF", marginBottom: 14 }}>المدفوعات حسب العميل</div>
                {sortedClients.map(c => {
                  ...
                  return (
                    <div key={c.id} style={{ marginBottom: 10, padding: 8, background: "#111827", borderRadius: 8 }}>
                      <div style={{ fontWeight: 600, color: "#FFFFFF", marginBottom: 4, fontSize: 12 }}>{c.name}</div>
              ...
              <div style={s.card}>
                <div style={{ fontSize: 13, fontWeight: 700, color: "#FFFFFF", marginBottom: 14 }}>المهام حسب التاغ</div>
                {tags.map((tg, i) => {
                  ...
                  return (
                    <div key={tg} style={{ display: "flex", justifyContent: "space-between", marginBottom: 8, alignItems: "center" }}>
                      ...
                      <span style={{ color: "#FFFFFF", fontSize: 12 }}>{count} مهمة</span>
                    </div>
                  );
                })}
              </div>
          ...
          <div style={{ maxWidth: 600, margin: "0 auto" }}>
            ...
            {managerMsg && (
              <div style={{ background: "linear-gradient(135deg, #1E3A5F, #2D2060)", border: "1px solid #60A5FA44", borderRadius: 14, padding: 24, marginBottom: 20, textAlign: "center" }}>
                ...
                <div style={{ fontSize: 12, color: "#FFFFFF", marginTop: 10 }}>رسالة من الإدارة</div>
              </div>
            )}
            {!managerMsg && (
              <div style={{ ...s.card, textAlign: "center", padding: 32, marginBottom: 20 }}>
                <div style={{ fontSize: 32, marginBottom: 8 }}>💬</div>
                <div style={{ color: "#FFFFFF", fontSize: 14 }}>لا توجد رسالة حالياً</div>
              </div>
            )}
            {/* نموذج تعديل الرسالة للمدير فقط */}
            {isAdmin && (
              <div style={s.card}>
                <div style={{ fontSize: 14, fontWeight: 700, color: "#FFFFFF", marginBottom: 14 }}>✏️ تعديل الرسالة</div>
                <textarea
                  rows={4}
        ...
        <div style={s.overlay} onClick={() => setSelectedTask(null)}>
          <div style={{ ...s.modal, maxWidth: 500 }} onClick={e => e.stopPropagation()}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 14 }}>
              ...
              <button onClick={() => setSelectedTask(null)} style={{ background: "none", border: "none", color: "#FFFFFF", fontSize: 18, cursor: "pointer" }}>✕</button>
            </div>
            <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 12 }}>
              ...
              {selectedTask.due && <span style={{ color: "#FFFFFF", fontSize: 12 }}>📅 {selectedTask.due}</span>}
            </div>
            {selectedTask.notes && <div style={{ background: "#0F172A", borderRadius: 8, padding: 10, fontSize: 12, color: "#FFFFFF", marginBottom: 12 }}>{selectedTask.notes}</div>}
            {selectedTask.createdBy && <div style={{ fontSize: 11, color: "#FFFFFF", marginBottom: 12 }}>أضافها: {selectedTask.createdBy} • آخر تعديل: {selectedTask.updatedBy || "-"}</div>}
            <div style={{ borderTop: "1px solid #1E293B", paddingTop: 12, marginBottom: 12 }}>
              <div style={{ fontSize: 13, fontWeight: 700, color: "#FFFFFF", marginBottom: 8 }}>📎 مرفقات المهمة</div>
            ...
            <div style={{ borderTop: "1px solid #1E293B", paddingTop: 12 }}>
              <div style={{ fontSize: 13, fontWeight: 700, color: "#FFFFFF", marginBottom: 10 }}>💬 التعليقات</div>
              <div style={{ maxHeight: 160, overflowY: "auto", marginBottom: 10 }}>
                {comments.filter(c => c.taskId === selectedTask.id).map(c => (
                  <div key={c.id} style={{ background: "#0F172A", borderRadius: 8, padding: 8, marginBottom: 6 }}>
                    ...
                    <div style={{ fontSize: 12, color: "#FFFFFF" }}>{c.text}</div>
                    <div style={{ fontSize: 10, color: "#FFFFFF", marginTop: 2 }}>{c.time?.toDate ? c.time.toDate().toLocaleString("ar-EG") : ""}</div>
                  </div>
                ))}
                {comments.filter(c => c.taskId === selectedTask.id).length === 0 && <div style={{ color: "#FFFFFF", fontSize: 12 }}>لا توجد تعليقات بعد</div>}
              </div>
              <div style={{ display: "flex", gap: 8 }}>
        ...
        <div style={s.overlay} onClick={() => setSelectedClient(null)}>
          <div style={{ ...s.modal, maxWidth: 560 }} onClick={e => e.stopPropagation()}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
              ...
              <button onClick={() => setSelectedClient(null)} style={{ background: "none", border: "none", color: "#FFFFFF", fontSize: 18, cursor: "pointer" }}>✕</button>
            </div>
            <div style={{ fontSize: 12, color: "#FFFFFF", marginBottom: 16 }}>
              {selectedClient.phone && <div>📞 {selectedClient.phone}</div>}
              {selectedClient.email && <div>📧 {selectedClient.email}</div>}
              {selectedClient.notes && <div style={{ marginTop: 6, color: "#FFFFFF" }}>{selectedClient.notes}</div>}
            </div>

            {/* سجل الأعمال */}
            <div style={{ marginBottom: 16 }}>
              <div style={{ fontSize: 13, fontWeight: 700, color: "#FFFFFF", marginBottom: 8 }}>📋 سجل الأعمال</div>
              {tasks.filter(t => t.clientId === selectedClient.id).map(t => (
                <div key={t.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "8px 10px", background: "#0F172A", borderRadius: 8, marginBottom: 6 }}>
                  <div>
                    <div style={{ fontSize: 12, color: "#FFFFFF", fontWeight: 600 }}>{t.title}</div>
                    <div style={{ fontSize: 10, color: "#FFFFFF" }}>{getEmp(t.empId)?.name} • {t.due}</div>
                  </div>
              ...
              {tasks.filter(t => t.clientId === selectedClient.id).length === 0 && <div style={{ color: "#FFFFFF", fontSize: 12 }}>لا توجد مهام</div>}
            </div>

            {/* سجل المدفوعات */}
            <div style={{ marginBottom: 16 }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
                <div style={{ fontSize: 13, fontWeight: 700, color: "#FFFFFF" }}>💰 سجل المدفوعات</div>
                <button
                  onClick={() => {
              ...
              {payments.filter(p => p.clientId === selectedClient.id).map(p => (
                <div key={p.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "8px 10px", background: "#0F172A", borderRadius: 8, marginBottom: 6 }}>
                  <div>
                    ...
                    <div style={{ fontSize: 10, color: "#FFFFFF" }}>{p.type} • {p.date}</div>
                  </div>
              ...
              {payments.filter(p => p.clientId === selectedClient.id).length === 0 && <div style={{ color: "#FFFFFF", fontSize: 12 }}>لا توجد مدفوعات</div>}
              <div style={{ marginTop: 10, padding: 10, background: "#0F172A", borderRadius: 8, display: "flex", justifyContent: "space-between" }}>
                <span style={{ color: "#FFFFFF", fontSize: 12 }}>إجمالي المدفوع</span>
            ...
            <div style={{ borderTop: "1px solid #1E293B", paddingTop: 14 }}>
              <div style={{ fontSize: 13, fontWeight: 700, color: "#FFFFFF", marginBottom: 8 }}>📎 مرفقات العميل</div>
              ...
              {attachments.filter(a => a.entityId === selectedClient.id && a.entityType === "client").length === 0 && (
                <div style={{ color: "#FFFFFF", fontSize: 12 }}>لا توجد مرفقات</div>
              )}
              {attachments.filter(a => a.entityId === selectedClient.id && a.entityType === "client").map(a => (
                <div key={a.id} style={{ display: "flex", alignItems: "center", gap: 8, padding: "7px 10px", background: "#0F172A", borderRadius: 8, marginBottom: 6 }}>
                  ...
                  <span style={{ fontSize: 10, color: "#FFFFFF", whiteSpace: "nowrap" }}>{a.uploadedBy}</span>
        ...
        <div style={s.overlay} onClick={() => setModal(null)}>
          <div style={s.modal} onClick={e => e.stopPropagation()}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
              ...
              <div>
                <div style={{ fontSize: 12, color: "#FFFFFF", marginBottom: 6 }}>التاغات:</div>
              ...
              <div style={{ background: "#0F172A", border: "1px solid #1E3A5F", borderRadius: 10, padding: 12 }}>
                ...
                {form.amount && <div style={{ fontSize: 11, color: "#FFFFFF", marginTop: 6 }}>سيتم إضافة هذا المبلغ تلقائياً لكشف المدفوعات</div>}
              </div>
              <div style={{ display: "flex", gap: 8, justifyContent: "flex-end" }}>
      ...
      {modal === "transfer" && (
        <div style={s.overlay} onClick={() => setModal(null)}>
          <div style={{ ...s.modal, maxWidth: 360 }} onClick={e => e.stopPropagation()}>
            ...
            <div style={{ fontSize: 13, color: "#FFFFFF", marginBottom: 12 }}>"{form.title}"</div>
            <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
        ...
        <div style={s.overlay} onClick={() => setModal(null)}>
          <div style={s.modal} onClick={e => e.stopPropagation()}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                <label style={{ color: "#FFFFFF", fontSize: 13 }}>اللون:</label>
      ...
      {undoToast && (
        <div style={{
          ...
          <button onClick={dismissUndo} style={{ background: "none", border: "none", color: "#FFFFFF", fontSize: 16, cursor: "pointer", padding: 0 }}>✕</button>
        </div>
      )}
      ...
      {managerMsg && tab !== "managermsg" && (
        <div style={{
          ...
          <div style={{ fontSize: 12, color: "#FFFFFF", lineHeight: 1.5, overflow: "hidden", display: "-webkit-box", WebkitLineClamp: 2, WebkitBoxOrient: "vertical" }}>{managerMsg}</div>
        </div>
      )}
    </div>
  );
