                ...
                <div key={status} style={{ ...s.card, minWidth: 240, flex: "0 0 240px" }}>
                  ...
                  <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                    {byStatus(status).map(task => {
                      ...
                      return (
                        <div key={task.id} className="task-card" style={{ background: "var(--bg-input, #111827)", border: `1px solid ${urg ? urg.color + "55" : "var(--border-alt, #1E293B)"}`, borderRadius: 10, padding: 12, cursor: "pointer" }} onClick={() => setSelectedTask(task)}>
                          ...
                          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginTop: 4 }}>
                            <div style={{ display: "flex", gap: 6 }}>
                              {task.due && <span style={{ fontSize: 9, color: urg ? urg.color : "var(--text-secondary, #FFFFFF)" }}>📅 {task.due}</span>}
              ...
              return (
                <div key={emp.id} style={{ ...s.card, border: `1px solid ${emp.color}33` }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 14 }}>
                    <div onClick={() => emp.photo && setLightboxImg(emp.photo)} style={{ width: 48, height: 48, borderRadius: "50%", overflow: "hidden", background: emp.color + "22", border: `2px solid ${emp.color}`, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, cursor: emp.photo ? "zoom-in" : "default" }}>
        ...
        <div style={s.overlay} onClick={() => setSelectedTask(null)}>
          <div style={{ ...s.modal, maxWidth: 500 }} onClick={e => e.stopPropagation()}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 14 }}>
              <h3 style={{ fontSize: 15, fontWeight: 700, color: "var(--text-primary, #F1F5F9)", margin: 0, flex: 1 }}>{selectedTask.title}</h3>
            ...
            <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 12 }}>
              ...
              {selectedTask.due && <span style={{ color: "var(--text-secondary, #FFFFFF)", fontSize: 12 }}>📅 {selectedTask.due}</span>}
            </div>
            {selectedTask.notes && <div style={{ background: "var(--bg-panel, #0F172A)", borderRadius: 8, padding: 10, fontSize: 12, color: "var(--text-secondary, #FFFFFF)", marginBottom: 12 }}>{selectedTask.notes}</div>}
            {selectedTask.createdBy && <div style={{ fontSize: 11, color: "var(--text-secondary, #FFFFFF)", marginBottom: 12 }}>أضافها: {selectedTask.createdBy} • آخر تعديل: {selectedTask.updatedBy || "-"}</div>}
